                ...
                grecaptcha.execute(document.getElementById('recaptchaSiteKey').value, {action: 'submit'}).then(async function(token) {
                    ...
                    const response = await fetch(`/api/v1/submission/${formSlug}`, {
                        ...
                        body: JSON.stringify({
                            ...
                            climbing_bouldering: fields['climbing_bouldering'],
                            climbing_rope: fields['climbing_rope'],
                            climbing_goals: fields['climbing_goals'],
                            gear: fields['gear'].filter((e) => e.checked).map((e) => e.value).join(','),
                            gear_harness_size: fields['gear_harness_size'],
                            gear_shoe_size: fields['gear_shoe_size'],
                            health_conditions: fields['health_conditions'],
                            health_accommodations: fields['health_accommodations'],
